...
void main() 
{
	...
	//BRDF
	float NDF = DistributionGGX(N, halfVec, roughness);   
    float G   = GeometrySmith(N, viewDir, lightDir, roughness);      
    vec3 F    = fresnelSchlick(max(dot(halfVec, viewDir), 0.0), F0);
	
	vec3 numerator    = NDF * G * F; 
    float denominator = 4.0 * max(dot(N, viewDir), 0.0) * max(dot(N, lightDir), 0.0) + 0.0001; // + 0.0001 to prevent divide by zero
	...
	//ibl ambient
	 F = fresnelSchlickRoughness(max(dot(N, viewDir), 0.0), F0, roughness);
