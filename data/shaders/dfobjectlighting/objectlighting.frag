...
float computeShadow(vec3 fragPos)
{
	float maxDist = 30.0;
	float minStep = 0.0;
	vec3 lightPos = inLightPos;//vec3(4.0,-3.0,4.0);
...
float computeLight(vec3 fragPos, vec3 normal)
{
	vec3 texCoord = worldToUV(fragPos);
	vec4 intex = texture(s_VoxelGrid, texCoord);
	float dist = intex.a;
	vec3 lightdir = intex.xyz;
...
void main() 
{
	vec4 vcolor = vec4(0.1,0.1,0.0,1.0);//texture(s_VoxelGrid, worldToUV(inPosition));//vec3(inUV, 0.5));
	float s = computeLight(inPosition, inNormal);
