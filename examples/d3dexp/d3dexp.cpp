...
public:
	...
	render::D3D12CommandBuffer m_dcommandBuffer;
	...
	{	
		...
		m_scissorRect = CD3DX12_RECT(0, 0, static_cast<LONG>(width), static_cast<LONG>(height));
		{
			...
			ThrowIfFailed(m_d3ddevice->CreateDescriptorHeap(&rtvoHeapDesc, IID_PPV_ARGS(&m_rtvoHeap)));
			...
			ThrowIfFailed(m_d3ddevice->CreateDescriptorHeap(&dsvoHeapDesc, IID_PPV_ARGS(&m_dsvoHeap)));
		...
		//planetable.Create(m_d3ddevice, m_srvHeap, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
		...
		// Create frame resources.
		{
			...
			CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(m_rtvoHeap->GetCPUDescriptorHandleForHeapStart(), 0, m_rtvDescriptorSize);
			...
			// Create SRV.
			//m_d3ddevice->CreateShaderResourceView(m_renderTargeto.Get(), nullptr, cbvSrvHandle);

			m_renderTargeto.Create(m_d3ddevice.Get(), TextureWidth, TextureHeight, render::GfxFormat::R8G8B8A8_UNORM, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
			m_renderTargeto.CreateDescriptor(m_d3ddevice.Get(), cbvSrvHandle, m_renderTargetoSRVHandleGPU, rtvHandle);
	...
	/*		D3D12_DEPTH_STENCIL_VIEW_DESC depthStencilDesc = {};
			...
			m_d3ddevice->CreateDepthStencilView(m_depthStencilOffscreen.Get(), &depthStencilDesc, m_dsvoHeap->GetCPUDescriptorHandleForHeapStart());*/

			D3D12_CPU_DESCRIPTOR_HANDLE emptyCPUHandle{};
			D3D12_GPU_DESCRIPTOR_HANDLE emptyGPUHandle{};
			m_depthStencilOffscreen.Create(m_d3ddevice.Get(), TextureWidth, TextureHeight, render::GfxFormat::D32_FLOAT, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL, D3D12_RESOURCE_STATE_DEPTH_WRITE);
			m_depthStencilOffscreen.CreateDescriptor(m_d3ddevice.Get(), emptyCPUHandle, emptyGPUHandle, m_dsvoHeap->GetCPUDescriptorHandleForHeapStart());
		}
		...
		pipelineOffscreen.Load(m_d3ddevice.Get(), GetAssetFullPath(L"shaders/d3dexp/shaders.hlsl"), "VSMain", "PSMainOffscreen", &odsl, props);
		props.depthBias = false;
		pipeline.Load(m_d3ddevice.Get(), GetAssetFullPath(L"shaders/d3dexp/shaders.hlsl"), "VSMain", "PSMain", &mdsl, props);
		pipelineMT.Load(m_d3ddevice.Get(), GetAssetFullPath(L"shaders/d3dexp/shaders.hlsl"), "VSMain", "PSMainMT", &pdsl, props);

		// Create the command list.
		//ThrowIfFailed(m_d3ddevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocator.Get(), pipeline.m_pipelineState.Get(), IID_PPV_ARGS(&m_commandList)));

		geometry.Load(m_d3ddevice.Get(), "./../data/models/venus.fbx", XMFLOAT3(0.0, -0.7, 0.0), 0.03f, m_dcommandBuffer.m_commandList.Get());
		planegeometry.Load(m_d3ddevice.Get(), "./../data/models/plane.obj", XMFLOAT3(0.0, planey, 0.0), 0.1f, m_dcommandBuffer.m_commandList.Get());
		...
		const UINT64 constantBufferSize = sizeof(SceneConstantBuffer);    // CB size is required to be 256-byte aligned.
		{
			...
			m_constantBuffer.Create(m_d3ddevice.Get(), sizeof(SceneConstantBuffer), &m_constantBufferData, cbvSrvHandle, cbvSrvHandleGPU);
		...
		//the other constant buffer
		{
			...
			m_constantBufferOffscreen.Create(m_d3ddevice.Get(), sizeof(SceneConstantBuffer), &m_constantBufferData2, cbvSrvHandle, cbvSrvHandleGPU);
		...
		m_texture.Load(m_d3ddevice.Get(), "./../data/textures/compass.jpg", m_dcommandBuffer.m_commandList.Get(), cbvSrvHandlet, cbvSrvHandleGPUt);
		...
		m_texture1.Load(m_d3ddevice.Get(), "./../data/textures/PlanksBare0002_1_S.jpg", m_dcommandBuffer.m_commandList.Get(), cbvSrvHandle1, cbvSrvHandleGPU1);
		...
		m_dcommandBuffer.End();
		ID3D12CommandList* ppCommandLists[] = { m_dcommandBuffer.m_commandList.Get() };
	...
	{
		Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> m_commandList = m_dcommandBuffer.m_commandList;
	...
	// fences to determine GPU execution progress.
		...
		modeltableoffscreen.Draw(m_commandList.Get());
		...
		modeltable.Draw(m_commandList.Get());
		...
		m_commandList->SetGraphicsRootDescriptorTable(1, cbvSrvHandle1);*/
		planetable.Draw(m_commandList.Get());
