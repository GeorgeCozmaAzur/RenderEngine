...
VulkanExampleBase::VulkanExampleBase(bool enableValidation)
{
	...
	bool libLoaded = engine::android::loadVulkanLibrary();
	assert(libLoaded);
#elif defined(_DIRECT2DISPLAY)
...
#if defined(VK_USE_PLATFORM_ANDROID_KHR)
	engine::android::loadVulkanFunctions(instance);
#endif
...
/*
	...
	submitInfo = engine::initializers::submitInfo();
			...
			case AINPUT_SOURCE_TOUCHSCREEN: {
				...
				switch (action) {
					case AMOTION_EVENT_ACTION_UP: {
						...
						if (eventTime - downTime <= engine::android::TAP_TIMEOUT) {
							float deadZone = (160.f / engine::android::screenDensity) * engine::android::TAP_SLOP * engine::android::TAP_SLOP;
					...
					case AMOTION_EVENT_ACTION_DOWN: {
						// Detect double tap
						int64_t eventTime = AMotionEvent_getEventTime(event);
						if (eventTime - vulkanExample->lastTapTime <= engine::android::DOUBLE_TAP_TIMEOUT) {
							float deadZone = (160.f / engine::android::screenDensity) * engine::android::DOUBLE_TAP_SLOP * engine::android::DOUBLE_TAP_SLOP;
							float x = AMotionEvent_getX(event, 0) - vulkanExample->touchPos.x;
							float y = AMotionEvent_getY(event, 0) - vulkanExample->touchPos.y;
							if ((x * x + y * y) < deadZone) {
