	...
	namespace render
	{
		...
		public:
...
#endif
			{
				...
				if (err != VK_SUCCESS) {
					engine::tools::exitFatal("Could not create surface!", err);
				}
				...
				{
					engine::tools::exitFatal("Could not find a graphics and/or presenting queue!", -1);
				}

				// todo : Add support for separate graphics and presenting queue
				if (graphicsQueueNodeIndex != presentQueueNodeIndex)
				{
					engine::tools::exitFatal("Separate graphics and presenting queues are not supported yet!", -1);
				}
			...
			*/
			void createDirect2DisplaySurface(uint32_t width, uint32_t height)
			{
				...
				if (!foundMode)
				{
					engine::tools::exitFatal("Can't find a display and a display mode!", -1);
					return;
				}
				...
				if (bestPlaneIndex == UINT32_MAX)
				{
					engine::tools::exitFatal("Can't find a plane for displaying!", -1);
					return;
				}
				...
				if (result != VK_SUCCESS) {
					engine::tools::exitFatal("Failed to create surface!", result);
				}
